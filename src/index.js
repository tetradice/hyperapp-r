...
var createNode = function(vdom, listener, isSvg, lifecycleProcs) {
  ...
  var created = (props.oncreate || props.created);
  if(created){
    lifecycleProcs.push(function(){
      created(node)
    })
  }
...
var removeChildren = function(element, vNode) {
  var props = vNode.props
  if(props) {
    for(var i = 0; i < vNode.children.length; i++) {
      var n = element.childNodes[i];
      if(n && n.nodeName === vNode.children[i].name){
        removeChildren(n, vNode.children[i])
      } else {
        if(console && console.warn) console.warn("[Hyperapp] The DOM element generated by Hyperapp has been added or removed from the outside. Under the element, onremove (removing) and ondestroy (destroyed) event may not be called.", { target: element });
      }
    }

    var destroyed = (props.ondestroy || props.destroyed)
    if(destroyed){
      destroyed(element)
    }
  }
...
function removeElement(parent, element, oldVNode) {
  function done() {
    parent.removeChild(removeChildren(element, oldVNode))
  }

  var cb = oldVNode.props && (oldVNode.props.onremove || oldVNode.props.removing)
  if (cb) {
    cb(element, done)
  } else {
    done()
...
var patch = function(parent, node, oldVNode, newVNode, listener, isSvg, lifecycleProcs) {
  if (oldVNode === newVNode) {
    ...
    node = parent.insertBefore(
      createNode((newVNode = getVNode(newVNode)), listener, isSvg, lifecycleProcs),
      node
    )
    if (oldVNode != null) {
      removeElement(parent, oldVNode.node, oldVNode)
    }
  } else {
    var tmpVKid
    var oldVKid
    ...
    var cb = oldVNode.type === RECYCLED_NODE ? (oldVProps.oncreate || oldVProps.created) : (oldVProps.onupdate || oldVProps.updated)
    if (cb) {
      lifecycleProcs.push(function () {
        cb(node, oldVProps)
      })
    ...
    if (oldHead > oldTail) {
      ...
      while (oldHead <= oldTail) {
        var v = oldVKids[oldHead++]
        removeElement(node, v.node, v)
      }
    } else {
      ...
      while (newHead <= newTail) {
        ...
        if (
          ...
          if (oldKey == null) {
            removeElement(node, oldVKid.node, oldVKid)
          }
          oldHead++
          continue
        }
      ...
      for (var i in keyed) {
        if (newKeyed[i] == null) {
          removeElement(node, keyed[i].node, keyed[i])
        }
      }
    }
  }
...
export var app = function(props) {
  ...
  var render = function() {
    ...
    while (lifecycleProcs.length) lifecycleProcs.pop()()
  }
